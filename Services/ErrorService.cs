    ...
    internal class ErrorService
    {
            ...
            )
        {
            ...
            foreach (Operation operation in operations)
            {
                ...
                {
                    var device = "M" + (startNum + alarmCount).ToString(); // 例: 01A01, 01A02, ...
                    var parameters = new DynamicParameters();

                    var comment = messages.FirstOrDefault(m => m.AlarmId == id)?.BaseMessage ?? string.Empty;
                    var alarm = messages.FirstOrDefault(m => m.AlarmId == id)?.BaseAlarm ?? string.Empty;
                    // 将来的にメッセージの代入処理を追加する。

                    parameters.Add("PlcId", plcId, DbType.Int32);
                    parameters.Add("CycleId", cycleId, DbType.Int32);
                    parameters.Add("Device", cycleId, DbType.String);
